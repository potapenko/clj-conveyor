...
Подход полезен при отложенных вычислениях, когда вы создаете несколько элементов очереди, но не имеете достаточно данных для вычисления в в  этот момент. Например, анимация - вы не знаете координаты элемент в будущем. Вы добавляете вызов "создать анимацию" в очередь, и когда он выполнится в будущем, будут добавлены элементы, создающее анимацию перемещения.  Важно что что анимация будет выполнена сразу после вызова этого вызова. 

## Другие методы

`play/stop` -  остановить конвейер

`played?` - проверить остановлен ли
